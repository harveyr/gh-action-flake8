...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const patterns = util_1.trimAndFilter(core.getInput('patterns').split(' '));
        const flake8Path = core.getInput('flake8-path') || 'flake8';
        let text = '';
        if (patterns.length) {
            yield kit.execAndCapture(flake8Path, ['--version']);
            const { stdout, stderr } = yield kit.execAndCapture(flake8Path, patterns, {
                failOnStdErr: false,
            });
            text = stdout + stderr;
        }
        const lines = util_1.trimAndFilter(text.split('\n'));
